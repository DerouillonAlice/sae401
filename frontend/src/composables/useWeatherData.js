...
export function useWeatherData(route, auth) {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (Array.isArray(data.list)) {
          ...
          dayHourlyData.value = hourlyByDay
        }
      } else {
  ...
  function getHourlyDataForDay(dayIndex) {
    if (dayIndex === 0) {
      return todayHourlyData.value
    }
    
    const dayData = forecastData.value[dayIndex - 1]
    if (!dayData) return []
    
    const dateKey = dayData.date.toDateString()
    const hourlyList = dayHourlyData.value[dateKey] || []
    
    if (hourlyList.length === 0) return []

    // Extraire matin, après-midi, soir des vraies données
    const findClosest = (targetHour, startHour, endHour) => {
      const candidates = hourlyList.filter(item => {
        const hour = new Date(item.dt * 1000).getHours()
        return hour >= startHour && hour < endHour
      })
      if (!candidates.length) return null
